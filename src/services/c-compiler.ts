...
export default class CompilerService {
    ...
    async runCode(code: string, entries: string[] | undefined) {
        ...
        try {
            ...
            const result = await axios.post(`http://${process.env.COMPILER_URL}:8001/`, {
            code,
            inputs: entries?.join(",")
            ...
            }else throw new appErrorConstructor(`Erro ao compilar seu programa da resolucao: ${axiosError.response?.data}`,500);
        }
